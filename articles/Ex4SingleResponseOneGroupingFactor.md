...
OneFactor() command has been written as a shortcut for a blind user who
wishes to obtain: - the counts of observations within each group, - the
mean, standard deviation and standard error for each group, - boxplots
and/or dotplots, - the one-way analysis of variance, and - Tukey’s
Honestly Significant Difference (HSD) test on the significance of the
...
The OneFactor() function can deliver all of this with minimal effort
from the user. In addition, the output HTML file is opened automatically
if using R interactively, giving the blind user immediate access to the
...
use of their screen reading software. All graphs included in the HTML
file can be presented using a text description available from the VI()
functionality of the BrailleR package.
