...
- removed some examples from help pages which have long run times. The content can be more easily accommodated in `pkgdown` pages
- accepted some proposed fixes via PR106 on GitHub
- fixed spelling errors and false positives. Word list file updated; Plenty of Rd, Rmd, and md files updated
...
# BrailleR 0.33.3
-Update author files
-Change the VI.ggplot output for unrecognised graphs to correct spelling of cannot
...
- moved a lot of message() and warning() calls to use specific functions instead. Should reduce the number of text strings that need translation.
...
- had to remove orientation checking for bars in `ggplot()` graphs; see VI.internals. this forced introduction of `rlanng` package dependency
...
# BrailleR 0.25.3
- WriteR() failed for JYS. Suggestion to use file.path() added to WriteR(). Proved insufficient so hard wired quote marks for command line being issued have been included. JYS reported success.
